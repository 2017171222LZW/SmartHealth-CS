    ...
    ui(new Ui::RegisterWindow)
{
    ...
    connect(&verify, UserVerify::registerSeccuss, this, this->registerSeccuss);
    connect(&verify, UserVerify::registerFail, this, this->registerFail);
}
...
void RegisterWindow::registerUser(const QByteArray &data)
{
    verify.registerUser(data);
}
...
void RegisterWindow::on_Register_clicked()
{
    ...
    if(ui->comboBox->currentIndex() == PAITIENT){   //用户注册
        //emit userRegister(person);
        JsonData data;
        data.addPerson(*person);
        data.addInt("function_id", REGISTER_FUNCTION_ID);
        qDebug() << QString(data.toByteArrayData()) << endl;
        registerUser(data.toByteArrayData());
        return;
    }
