...
void LoginWindow::connect_Regi_Veri(){
    ...
    connect(registerWin, RegisterWindow::userRegister, verify, verify->verifyRegister);
    connect(verify, verify->registerSeccuss, registerWin, registerWin->registerSeccuss);
    connect(verify, verify->registerFail, registerWin, registerWin->registerFail);
}
/**
 * @功能: 获取注册界面对象
