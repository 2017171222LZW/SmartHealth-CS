...
int change_register_user(user_register_t *user_register)
{
	int ret = 0;
	int len = 0;	
	char *errmsg;
	...
	sprintf(sql,"update ser_info set passowrd = %s where id = %s",
			user_register->password,\
			user_register->id);	
...
int change_password_userid(const char *password,const char *userid)
{
	int ret;
	int ok = 1;
	char sql[1024];
	...
	//select passowrd from user_info where telphone='123409' and id='212312';	
	sprintf(sql,"update user_info set passowrd=%s where id=%s",password,userid);	
	
	printf("sql : %s\n",sql);
	ret = sqlite3_exec(db_handler,sql,NULL,NULL,&errmsg);
	if(ret != SQLITE_OK){
		printf("Fail to sqlite3_exec,error:%s\n",errmsg);
		pthread_mutex_unlock(&thread_lock);
		sqlite3_close(db_handler);
		return -1;
	}
