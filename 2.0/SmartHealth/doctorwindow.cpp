    ...
    ui(new Ui::DoctorWindow)
{
    ...
    connect(verify, verify->idInformationReady, appointWindow, appointWindow->displayInformation);
}
...
void DoctorWindow::on_doctorTable_cellClicked(int row, int column)
{
    qDebug() << row << ':' << column;
}
