...
ChatOnlineWindow *FunctionWindow::getChartOLWindow()
{
    if(NULL == this->chartOnlineWin){
        this->chartOnlineWin = new ChatOnlineWindow(verify);
        //connect(chatWindow, chatWindow->back2Func, this, backFromData);
    }
    return chartOnlineWin;
}
...
void FunctionWindow::on_Login_5_clicked()
{
    getChartOLWindow()->show();
}
